...
public class DictionaryNace extends Dictionary {
    ...
    public void addMappings(XMLStreamWriter out, String id) throws IOException, XMLStreamException {
        ...
        out.writeAttribute(
                "rdf:resource", "http://rdfdata.eionet.europa.eu/eurostatdic/nace_r2/" + id);
        out.writeEndElement();

        if (id.length() == 1) {
            out.writeStartElement("owl:sameAs");
            out.writeAttribute(
                    "rdf:resource", "http://ec.europa.eu/eurostat/ramon/rdfdata/nace_r2/" + id);
            out.writeEndElement();
        } else if (id.length() == 3) {
            ...
            try {
                ...
                out.writeAttribute(
                        "rdf:resource",
                        "http://ec.europa.eu/eurostat/ramon/rdfdata/nace_r2/" + id.substring(1));
                out.writeEndElement();
