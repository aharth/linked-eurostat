...
public class ClServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        int clIndex = id.indexOf("/cl/");
        if (clIndex == -1) {
            resp.sendError(HttpServletResponse.SC_NOT_FOUND);
            return;
        }
        id = id.substring(clIndex + "/cl/".length());

        URL url = new URL(Main.URI_PREFIX_3 + "/structure/codelist/ESTAT/" + id);

        ServletContext ctx = getServletContext();
        Transformer t = (Transformer) ctx.getAttribute(Listener.CL_T);
        ...
        try {
            ...
            resp.sendError(500, "Error processing codelist for " + id + ": " + e.getMessage());
            e.printStackTrace();
            return;
